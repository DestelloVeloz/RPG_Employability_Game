 ...
 * level = the level of the enemy. if FFP_EnemyLevels.js is not installed, the
...
Imported.FFP_PokemonBaseStats = true;

var FFP = FFP || {};
FFP.BaseStats = FFP.BaseStats || {};
FFP.BaseStats.parameters = PluginManager.parameters('FFP_BaseStats');
...
Game_BattlerBase.prototype.setParamIv = function() {
	...
	if (eval(String(FFP.BaseStats.parameters['UseIv']))) {
		var maxIv = parseInt(eval(String(FFP.BaseStats.parameters['MaxIv'])));
		for (var i = 0; i < 8; i++) {
...
Game_BattlerBase.prototype.param = function(paramId) {
	if (this.isActor()) {
		...
		if (paramId <= 1) {
			if (paramId == 0 && shedinja) {
				newCalc = 1;
			} else {
				newCalc = parseInt(eval(String(FFP.BaseStats.parameters['HPandMPFormula'])));
			};
		} else {
			newCalc = parseInt(eval(String(FFP.BaseStats.parameters['OtherStatsFormula'])));
		};
		if (!(shedinja && paramId == 0)) {
			newCalc *= this.paramRate(paramId) * this.paramBuffRate(paramId);
			newCalc = Math.round(newCalc.clamp(minValue, maxValue));
		};
		return newCalc;
	} else {
	    var maxValue = this.paramMax(paramId);
		...
		if (Imported.FFP_EnemyLevels) {
			level = this.level();
		} else if (eval(FFP.BaseStats.parameters['AverageLevel'])) {
		...
		newCalc *= this.paramRate(paramId) * this.paramBuffRate(paramId);
		return Math.round(newCalc.clamp(minValue, maxValue));
	};
	return 0;
};
...
Game_Actor.prototype.maxEv = function() {
	return parseInt(eval(String(FFP.BaseStats.parameters['maxEv'])));
};