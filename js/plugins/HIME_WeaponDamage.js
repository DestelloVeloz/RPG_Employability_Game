...
-------------------------------------------------------------------------------
@plugindesc v1.2 - Define weapon damage formulas and separate them from
...
(function ($) {
  ...
  Game_Battler.prototype.evalWeaponDamage = function(weapon) {
  ...
  Game_Battler.prototype.weaponDamage = function() {    
    var weapons = [];
    if (this.weapons) {
      weapons = this.weapons();
    }
    if (weapons.length === 0) {      
      return this.evalWeaponDamage($dataWeapons[$.barehandId]);
    }
    else {
      var total = 0;
      for (var i = 0; i < weapons.length; i++) {
        total += this.evalWeaponDamage(weapons[i]);
      }    
      return total;
    }
  };
