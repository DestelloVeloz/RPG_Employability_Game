...
Imported['VE - Fog And Overlay'] = '1.02';
...
(function() {
	...
	DataManager.loadDatabase = function() {
		...
		PluginManager.requiredPlugin.call(PluginManager, 'VE - Light Effects', 'VE - Basic Module', '1.06');
	};
...
/*:
 *------------------------------------------------------------------------------ 
 * @plugindesc v1.02 - Display fogs and overlay on maps and battles.
 * @author Victor Sant
 *
 * @param Max Fogs
 ...
 *    depth: n   : fog depth (0-7, see details bellow)
 *
 * ------------------------------------------------------------------------------
 *  Plugin Commands
 * ------------------------------------------------------------------------------
 ...
 *      depth   : fog depth (0-7, see details bellow)
 *
 ...
 *  the battle and battle only sufix.
 *
 * ------------------------------------------------------------------------------
 *
 * Example Notetags:
...
(function() {
	...
	Game_Screen.prototype.changeZoom = function(fogId, zoom, duration) {
		var fog = this.fog(fogId);
		if (fog) fog.changeZoom(zoom, duration);
	};

	Game_Screen.prototype.changeOpacity = function(fogId, opacity, duration) {
		var fog = this.fog(fogId);
	...
	Game_Map.prototype.setupFog = function(note) {
		...
		var name  = String(this.fogValue(note, new RegExp("name:[ ]*[\"'“‘](.*)[\"'”’]",'gi'), ''));
	...
	Game_Interpreter.prototype.pluginCommand = function(command, args) {
		...
		if (command.toLowerCase() === 'fogopacity') {
			...
			$gameScreen.changeOpacity(id, op, d)
		}
		if (command.toLowerCase() === 'fogzoom') {
			var id = Number(args[0]);
			var z  = Number(args[1]);
			var d  = Number(args[2]) || 0;
			$gameScreen.changeZoom(id, z, d)
		}
	};
	...
	Sprite.prototype._sortChildren = function() {
	...
	Object.defineProperties(Sprite_Actor.prototype, {
		z: { get: function() { return 3; }, configurable: true }
	});
	...
	Object.defineProperty(Game_Fog.prototype, 'z', {
		get: function() {
			var values = [10, 9, 6, 4, 3.5, 2, 1, -1];
			return values[this._depth] || 10;
		}, 
		configurable: true 
	});
